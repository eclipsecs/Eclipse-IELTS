        ...
        {q.options?.map(opt => (
          <button
            ...
            className={`flex items-center gap-4 w-fit p-2 pr-6 rounded-lg border-2 transition-all ${
              userAnswers[q.id] === opt.value ? 'border-blue-500 bg-blue-500/5' : 'border-transparent hover:bg-slate-500/5'
            }`}
          >
            <span className={`text-sm font-black w-6 h-6 rounded flex items-center justify-center border-2 ${
              userAnswers[q.id] === opt.value ? 'bg-blue-500 border-blue-500 text-white' : isDarkMode ? 'border-slate-700 text-slate-400' : 'border-slate-200 text-slate-600'
            }`}>{opt.value}</span>
            <span className={`text-sm font-bold ${userAnswers[q.id] === opt.value ? 'text-blue-500' : isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>{opt.label}</span>
          </button>
        ))}
      </div>
    </div>
