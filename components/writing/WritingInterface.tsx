...
import { WRITING_TASK1_PROMPTS } from './task1/Prompts';
import { WRITING_TASK2_PROMPTS } from './task2/Prompts';
...
const WritingInterface: React.FC<WritingInterfaceProps> = ({
  ...
  const promptData = WRITING_TASK1_PROMPTS[testId] || WRITING_TASK2_PROMPTS[testId];
  ...
  return (
    <div className={`flex h-full w-full divide-x transition-colors relative ${isDarkMode ? 'divide-gray-800' : 'divide-gray-300'}`}>
          ...
          <div className={`group relative w-full border-y transition-all ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-slate-100'}`}>
            {imageUrl ? (
              <div className="w-full flex justify-center bg-white p-6">
                <img src={imageUrl} alt="Stimulus" className="w-full h-auto block object-contain max-h-[800px]" />
              </div>
            ) : (
